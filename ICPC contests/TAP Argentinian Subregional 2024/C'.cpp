...
/* ld planeDist(P a, P b, P c, P p) {
  P n = (b-a).cross(c-a);
  return (ld)n.dot(p-a) / n.dist();
} */
...
bool solve(P cus, P sun, vector<P>& poly) {
  ll N = poly.size();

  fore(i, 0, N) {
    P p = poly[i], q = poly[(i + 1) % N];
...
int main() {
  ...
  bool ans = solve(P(X_cus, Y_cus, Z_cus), P(X_sol, Y_sol, Z_sol), poly);
